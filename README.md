...
### 2. LLM 主动记忆系统 (LLM-Driven Memory)
传统 Agent 记忆依赖关键词匹配，覆盖不全且误判率高。Syntropy 将记忆捕获决策权交给 LLM：

*   **智能判断**: Agent 在对话中自主识别值得记忆的信息（个人偏好、项目决策、重要事实），主动调用 `save_memory` 工具持久化。
*   **语义分类**: LLM 根据上下文自动选择记忆类别（personal/preference/project/decision），无需人工标注。
*   **记忆管理**: 前端 **MemoryVault (记忆库)** 组件提供搜索、分类过滤、编辑和删除能力，形成完整的记忆管理闭环。

### 3. 基于 RRF 的混合记忆引擎 (Hybrid Memory Engine with RRF)
...
### 4. 内核级状态机 (Kernel-Level State Machine)
...
### 5. 人机协同”御批”协议 (Human-in-the-loop Protocol)
...
### 6. 结构化可观测性 (Structured Observability)
通过 **Tracer** 模块实现全链路追踪，每次用户指令生成唯一 `traceId`，贯穿 Agent 调度、工具调用、LLM 推理全流程。

*   **8 种诊断事件**: `agent.turn`、`tool.call`、`model.usage`、`dispatch`、`approval.wait`、`agent.stuck` 等结构化事件，便于性能分析和故障排查。
*   **日志脱敏**: 自动截断 API Key 等敏感信息，保护系统安全。
*   **卡死检测**: 3 分钟无响应自动告警，防止 Agent 永久挂起。
...
*   **奏折阁 (Imperial Archives)**:
    ...
    *   **折叠/展开**: 每份奏折支持折叠，仅展示摘要；展开后可查看完整对话链路与决策树。
    *   **多视角叙事**: 左侧展示百官的回复与思考，右侧展示皇帝（用户）的指令，清晰还原对话脉络。

*   **记忆库 (Memory Vault)**:
    *   **长期记忆管理**: 展示 Agent 主动保存的重要信息（个人偏好、项目决策、关键事实）。
    *   **搜索与过滤**: 支持关键词搜索和按类别（personal/preference/project/decision）筛选。
    *   **在线编辑**: 支持直接在前端编辑或删除记忆条目，实时同步到后端。
...
| | **Vitest + Testing Library** | 前端单元测试与组件测试 |
| **Backend** | **Node.js (Express)** | 稳健的后端服务与 RESTful API |
...
│   │   ├── Console/    # 控制台模块 (MemorialsPanel, MemoryVault, OfficialsPanel)
│   │   └── Dashboard/  # 仪表盘布局
│   ├── game/           # Phaser 游戏核心 (MainScene, Agent Sprites)
│   ├── store/          # Zustand 状态管理 (React-Phaser Bridge)
│   ├── services/       # 业务服务 (Socket, LiveAgentService)
│   └── test/           # 前端测试 (Vitest + @testing-library/react)
├── server/             # 后端源码
│   ├── core/           # 智能体内核 (Kernel, Agent, LLM)
│   ├── runtime/        # 运行时 (MemoryManager, ContextManager, SocketGateway)
│   ├── infra/          # 基础设施 (Tracer, SessionStore, Logger)
│   ├── config/         # 官员配置 (officials.json)
│   └── index.js        # 服务入口
├── skills/             # Agent 技能库 (save_memory, call_officials, etc.)
├── data/               # 数据持久化 (SQLite DBs, Agent workspaces)
├── docs/               # 项目文档 (ARCHITECTURE.md, etc.)
└── test/               # 后端测试 (run_tests.js)
```
